<?php
if (is_array($suadm)) {
    extract($suadm);
}
?>
<section class="main_content dashboard_part">
    <div class="main_content_iner ">
        <div class="container-fluid p-0">
            <div class="row justify-content-center">
                <div class="col-12">
                    <div class="dashboard_header mb_50">
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="dashboard_header_title">
                                    <h3>Cập Nhật Loại Quần</h3>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-12">
                    <div class="white_box mb_30">
                        <div class="main-title">
                            <h3 class="mb-0">Loại Quần</h3>
                        </div>
                        <div class="modal-body ">
                            <form action="index.php?act=updatedm" method="post">
                                <div class="mb_10">
                                    <label for="">
                                        <div class="main-title">
                                            <h5 class="mb-0">Mã Loại Quần:</h5>
                                        </div>
                                    </label>
                                    <input type="text" class="form-control" name="maloai" disabled placeholder="auto number">
                                </div>
                                <div class="mb_10">
                                    <label for="">
                                        <div class="main-title">
                                            <h5 class="mb-0">Tên Loại Quần:</h5>
                                        </div>
                                    </label>
                                    <input type="text" class="form-control" name="tenloai" value="<?php if (isset($name_category) && ($name_category != "")) echo $name_category; ?>" placeholder="Tên loại quần">
                                    <?php if ((isset($errortenloai))) { ?>
                                        <span style="color: red;"><?= $errortenloai ?></span>
                                    <?php } ?>
                                </div>
                                <div class>
                                    <div class="add_button">
                                        <input type="hidden" name="id_category" value="<?php if (isset($id_category) && ($id_category > 0)) echo $id_category; ?>">
                                        <input type="submit" name="capnhat" class="btn_1" value="Cập Nhật">
                                        <input type="reset" class="btn_1" value="Nhập Lại">
                                    </div>
                                </div>
                            </form>
                            <?php
                            if (isset($thongBao) && ($thongBao != "")) {
                                echo $thongBao;
                            }

                            ?>
                        </div>
                    </div>
                </div>
            </div>
        </div>